...
if {[validate_sim_top $top_module_sim]} {
    ...
    #开启动态仿真，如果要静态仿真请注释该代码
    set_property -name {vcs.compile.vlogan.more_options} -value {-kdb -lca} -objects [get_filesets sim_1]
    # 添加编译参数-kdb -debug_access+all ，设置仿真单位1ns/1ps
    set_property -name {vcs.elaborate.debug_acc} -value {true} -objects [get_filesets sim_1]
    set_property -name {vcs.elaborate.vcs.more_options} -value {-debug_access+all -timescale=1ns/100ps} -objects [get_filesets sim_1]
    # 设置仿真参数
    ...
    # export_simulation  -lib_map_path "/tools/Xilinx/Vivado/2024.2/vcs_lib" -absolute_path -force -directory "$base_dir" -simulator vcs  -ip_user_files_dir "$base_dir/prj/Script_test.ip_user_files" -ipstatic_source_dir "$base_dir/prj/Script_test.ip_user_files/ipstatic" -use_ip_compiled_libs
